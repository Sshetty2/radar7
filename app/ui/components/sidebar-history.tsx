...
import {
  ...
  ShareIcon,
  TrashIcon
} from '@/app/ui/components/base/icons';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { useChatVisibility } from '@/app/ui/hooks/use-chat-visibility';
...
export function SidebarHistory ({ user }: { user: User | undefined }) {
  ...
  const {
    ...
    isLoading,
    mutate
  } = useSWR<Chat[]>(user ? '/api/history' : null, fetcher, { fallbackData: [] });
  ...
  const handleDelete = () => {
    ...
    toast.promise(deletePromise, {
      ...
      success: () => {
        mutate(_history => {
          if (_history) {
            return _history.filter(_ => _.id !== id);
          }
        });
